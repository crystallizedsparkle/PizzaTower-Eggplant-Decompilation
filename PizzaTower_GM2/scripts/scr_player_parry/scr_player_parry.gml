function scr_player_parry()
{
    ...
    if (parry_count > 0)
    {
        ...
        with (obj_baddie)
        {
            if (distance_to_object(other) < parry_threshold && state != states.hit && state != states.grabbed && state != states.stun && parryable && !(state == states.stun && thrown))
            {
                notification_push(notification_type.unknown1, [id, object_index, room]);
